package io.github.yusufsdiscordbot.yusufsdiscordcore.bot.handlers.adapter;
...
public abstract class YusufListenerAdapter implements EventListener {
    ...
    // Interaction Events
    public void onSlashCommandInteraction(@Nonnull YusufSlashCommandInteractionEvent event) {}

    public void onUserContextInteraction(@Nonnull YusufUserContextInteractionEvent event) {}

    public void onMessageContextInteraction(@Nonnull YusufMessageContextInteractionEvent event) {}
    ...
    public final void onEvent(@Nonnull GenericEvent event) {
        ...
        for (Class<?> clazz : ClassWalker.range(event.getClass(), GenericEvent.class)) {
            if (unresolved.contains(clazz))
                continue;
            MethodHandle mh = methods.computeIfAbsent(clazz, YusufListenerAdapter::findMethod);
            if (mh == null) {
    ...
    private static @Nullable MethodHandle findMethod(@NotNull Class<?> clazz) {
        String name = clazz.getSimpleName();
        MethodType type = MethodType.methodType(Void.TYPE, clazz);
        try {
            name = "on" + name.substring(0, name.length() - "Event".length());
            return lookup.findVirtual(YusufListenerAdapter.class, name, type);
